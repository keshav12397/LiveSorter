...
// Runs CalibrateAllUnits.exe instead of the Python. Same five output files,
// no Python/numpy/scipy needed on the rig.
//
// It takes a CONFIG FILE, not flags, and its key names differ from the live
// config's (ksDir vs trainingKsDir, outDir vs filterDir, nChannels vs
// filterNChannels, ...). So this writes a small translated config next to
// the output directory rather than trying to share one file -- renaming the
// keys in either binary to match the other would be a breaking change to a
// working tool for cosmetic reasons.
void runCppCalibrationAllUnits( const Config &cfg, const std::string &filterDir )
{
    std::string exe = resolveRelativeToExe(
        cfg.getString( "calibrateAllUnitsExe", "CalibrateAllUnits.exe" ) );

    // The generated config lands in filterDir, which the calibrator creates
    // anyway, and is left behind on purpose: it is the exact input that
    // produced the bank sitting beside it.
    std::string cfgPath = filterDir + "/_calibrate_all_units.generated.txt";

#ifdef _WIN32
    CreateDirectoryA( filterDir.c_str(), NULL );
#endif

    std::ofstream fh( cfgPath.c_str() );
    if( !fh.is_open() )
        throw std::runtime_error( "could not write " + cfgPath );

    fh << "# Generated by ClosedLoopAllUnits (calibrationBackend=cpp).\n"
       << "# Overwritten on every calibrating run; edit the live config, not this.\n"
       << "ksDir="          << cfg.requireString( "trainingKsDir" )   << "\n"
       << "binPath="        << cfg.requireString( "trainingBinPath" ) << "\n"
       << "channelMapJson=" << cfg.requireString( "carChannelMapJson" ) << "\n"
       << "outDir="         << filterDir << "\n"
       << "nChannels="      << cfg.getInt( "filterNChannels", 5 ) << "\n"
       << "templateLength=" << cfg.getInt( "templateLength", 61 ) << "\n"
       << "templateOffset=" << cfg.getInt( "templateOffset", 20 ) << "\n"
       << "trainFrac="      << cfg.getDouble( "calibrationTrainFrac", 0.5 ) << "\n"
       << "ridge="          << cfg.getDouble( "calibrationRidge", 1e-3 ) << "\n"
       << "maxSpikes="      << cfg.getInt( "calibrationMaxSpikes", 2000 ) << "\n"
       << "applyHighpass="  << ( cfg.getBool( "applyHighpass", true ) ? "true" : "false" ) << "\n"
       << "highpassCutoffHz=" << cfg.getDouble( "highpassCutoffHz", 300.0 ) << "\n"
       << "nThresholds="    << cfg.getInt( "calibrationNThresholds", 60 ) << "\n"
       << "autoInterferers=" << cfg.getInt( "autoInterferers", 5 ) << "\n"
       << "minSpikes="      << cfg.getInt( "calibrationMinSpikes", 200 ) << "\n"
       << "maxUnits="       << cfg.getInt( "calibrationMaxUnits", 0 ) << "\n"
       << "seed="           << cfg.getInt( "calibrationSeed", 0 ) << "\n";

    std::string metaPath = cfg.getString( "trainingMetaPath", "" );
    if( !metaPath.empty() )
        fh << "metaPath=" << metaPath << "\n";
    std::string scratchPath = cfg.getString( "calibrationScratchPath", "" );
    if( !scratchPath.empty() )
        fh << "scratchPath=" << scratchPath << "\n";
    fh.close();
    ...
    if( rc != 0 )
        throw std::runtime_error(
            "\'" + exe + "\' exited with code " + std::to_string( rc ) );
}
...
int main( int argc, char **argv )
{
    ...
    try {
        ...
        if( !skipCalibration ) {
            ...
            // Two backends, same five output files. `python` is the
            // default because FilterGen is the single implementation of the
            // fit and sweep math; `cpp` runs CalibrateAllUnits.exe instead
            // and needs no Python on the rig.
            //
            // They are NOT known to produce identical banks. Every numerical
            // piece the C++ calls is validated against Python individually
            // (see the equivalence tests), but nothing compares the two
            // calibrations end to end, and candidate selection, interferer
            // picking and RNG draws all sit outside those tests. Treat a
            // backend switch as a recalibration, not a no-op.
            std::string backend = cfg.getString( "calibrationBackend", "python" );
            if( backend != "python" && backend != "cpp" ) {
                throw std::runtime_error(
                    "Config: calibrationBackend must be 'python' or 'cpp', got '"
                    + backend + "'" );
            }
            std::cout << "Calibrating all units into " << filterDir
                       << " (backend=" << backend << ") ...\n";
            if( backend == "cpp" )
                runCppCalibrationAllUnits( cfg, filterDir );
            else
