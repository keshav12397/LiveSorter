...
//
// Same arrangement the single-target path uses (main.cpp's
// runPythonCalibration): FilterGen is the ONE implementation of "fit a
// filter" and "score a threshold". A C++ port of this existed and was
// removed -- measured against the script it selected different channels on
// 3 of 4 units and produced a filter correlating at 0.758 on the fourth,
// while being no faster and no lighter. See REMOVED.md.
...
int main( int argc, char **argv )
{
    ...
    try {
        ...
        // (FilterGen/calibrate_all_units.py), because the extraction window has to
