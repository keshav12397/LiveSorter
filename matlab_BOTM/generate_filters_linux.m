...
% User-defined
CAR = true;         % Apply CAR before BOTM?
...
% Get the raw data
if isfield(sp, 'dat_path')
    raw_path = sp.dat_path;
else
    disp("Select raw data.")
    [f, p] = uigetfile();
    raw_path = fullfile(p, f);
end

% Raw data
s = split(raw_path, ".");                     % Remove
...
%% Remove Bad Channels
% Look for the file
raw_dir = split(np_file_name, '/');
raw_dir = raw_dir(1:(end-1)); 
raw_dir = char(join(raw_dir, '/'));
bad_ch_file = fullfile(raw_dir, 'bad_channels.tsv');

% If the file exists, get rid of the bad channels
if isfile(bad_ch_file)
    bad_ch = readmatrix(bad_ch_file, 'FileType', 'text', 'Delimiter', '\t');
    neural(:, ismember(np_ch, bad_ch)) = [];
    np_ch(ismember(np_ch, bad_ch)) = [];
end
...
%% Perform CAR
% If desired, do common average referencing (CAR)
if CAR
    ref = median(neural, 2);
    neural = neural - repmat(ref, [1, size(neural, 2)]);
end
